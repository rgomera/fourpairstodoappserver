...
// count unfinish private todos base on specific user
app.get('/todos/count_privatetodos/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const data = [id];

        const query_response = await pool.query('SELECT count_uprivate($1) AS  "unfinish_private"', data);
        res.json(query_response.rows[0]);
    } catch (err) {
        console.error(err.message);
    }
...
// count unfinish all public todos
app.get('/todos/count_publictodos/', async (req, res) => {
    try {
        const query_response = await pool.query('SELECT * FROM count_upublic AS "unfinish_public"');
        res.json(query_response.rows[0]);
    } catch (err) {
        console.error(err.message);
    }
